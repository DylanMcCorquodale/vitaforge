...
import { validateDailyLog } from "../src/validation";
import { ConflictError, UnauthorizedError } from "./errors";
import type { DailyLogInput } from "../src/types";
...
export async function register(input: Record<string, unknown>) {
  ...
  try {
    ...
    if (duplicate(error)) throw new ConflictError("An account already exists for this email.");
    throw error;
  }
...
export async function userForRequest(request: Request) {
  const token = requestToken(request);
...
export async function createLog(userId: string, input: Record<string, unknown>) {
  const db = await getDatabase();
  ...
  try {
    ...
    if (duplicate(error)) throw new ConflictError("A log already exists for this date. Edit the existing entry instead.");
    throw error;
  }
  return serializeLog(log);
}

export async function updateLog(userId: string, logId: string, input: Partial<DailyLogInput>) {
  const db = await getDatabase();
  ...
  try {
    ...
    if (duplicate(error)) throw new ConflictError("A log already exists for this date.");
    throw error;
  }
}

export async function removeLog(userId: string, logId: string) {
  const db = await getDatabase();
