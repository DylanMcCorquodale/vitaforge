...
const databaseName = process.env.MONGODB_TEST_DB || process.env.MONGODB_DB || "vitaforge";
...
function connection() {
  ...
  if (!clientPromise) {
    ...
    clientPromise = client.connect().catch((error) => {
      ...
      return client.close().catch(() => undefined).then(() => {
        throw error;
      });
    });
  }
...
export async function closeDatabaseForTests() {
  if (clientPromise) {
    ...
    clientPromise = undefined;
    await pendingClient.then((client) => client.close(), () => undefined);
  }
  indexesPromise = undefined;
}
