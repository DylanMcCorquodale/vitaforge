import type { DailyLogInput, WorkoutIntensity } from "./types";
...
export function finiteNumber(value: unknown, field: string, { min = -Infinity, max = Infinity, integer = false }: { min?: number; max?: number; integer?: boolean } = {}) {
...
export function validateDailyLog(input: Record<string, unknown>): DailyLogInput {
  const intensities = new Set<WorkoutIntensity>(["Easy", "Moderate", "Hard", "Rest"]);
  if (!intensities.has(input.workoutIntensity as WorkoutIntensity)) throw new ValidationError("Choose a valid workout intensity.");
  ...
  return {
    ...
    workoutIntensity: input.workoutIntensity as WorkoutIntensity,
